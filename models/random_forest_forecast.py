...
def random_forest_forecast(data, period, n_years, selected_name, confidence_interval=None):
    ...
    with st.container():
        ...
        fig.add_trace(go.Scatter(x=future_dates, y=future_predictions, mode='lines', name='RF Forecast',
                                 line=dict(color='#2ca02c', width=3)), row=1, col=1)
        errors = y_true_all - y_pred_all
        fig.add_trace(go.Scatter(x=dates_all, y=errors, mode='lines', name='Error',
        ...
        info = f"Features used: {len(X.columns)} | Cross-validation folds: 5"
